...
class FeatureEngineer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'age' in X_transformed.columns and 'thalach' in X_transformed.columns:
            ...
            X_transformed['hr_reserve'] = (220 - X_transformed['age']) - X_transformed['thalach']
        
        # Cholesterol to age ratio
        if 'chol' in X_transformed.columns and 'age' in X_transformed.columns:
            X_transformed['chol_age_ratio'] = X_transformed['chol'] / X_transformed['age']
        ...
        if self.create_interactions:
            if 'exang' in X_transformed.columns and 'oldpeak' in X_transformed.columns:
                X_transformed['exang_oldpeak'] = X_transformed['exang'] * X_transformed['oldpeak']
            
            if 'cp' in X_transformed.columns and 'thalach' in X_transformed.columns:
                X_transformed['cp_thalach'] = X_transformed['cp'] * X_transformed['thalach']
...
def create_full_pipeline(
    ...
    # Preprocessing
    steps.append(('preprocessor', create_preprocessing_pipeline(scaling_method=scaling_method)))
